...
function init() {
  ...
  const singleSvgWrapper = ({ content, color, w, h }) => {
    return `<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100%" height="100%" viewBox="0 0 ${w} ${h}" fill="${
      color ? color : "black"
    }">${content}</svg>`;
  };

  const transformPos = ({ target, x, y, value }) => {
    target.style.transform = `translate(${x || 0}${value || "px"},${y || 0}${
      value || "px"
    })`;
  };
  ...
  const animationTiming = () => {
    const total = nearestN(
      elements.sheepRoute.getBoundingClientRect().width * 10,
      1000
    );
    return {
      jump: total * 0.26 - 200,
  ...
  const filteredSheepData = (n) =>
    sheepData.data.filter((s) => s.sheepId === n)[0];
  const baaa = () =>
    `b` +
    new Array(1 + randomN(3))
      .fill("")
      .map((_) => "a")
      .join("");
